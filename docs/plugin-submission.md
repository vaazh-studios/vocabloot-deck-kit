...
## Claude Code: the community marketplace

**Submitted for review 2026-09-16** through the Console form <https://platform.claude.com/plugins/submit> (the claude.ai form needs a Team or Enterprise organization; the Console form takes an individual org). What it asked for: the repository link (root, no subpath), a homepage, the plugin name `vocabloot-deck-kit` (immutable once listed), a description, example use cases, supported platforms (Claude Code ticked; Claude Cowork left unticked because the form says to test on each surface first and the kit has not run there), licence, privacy policy URL, contact email. Status: <https://platform.claude.com/plugins> ("View submissions").

After approval the plugin lands in `anthropics/claude-plugins-community`, pinned to a commit that Anthropic's CI bumps as the repo is pushed; the catalog syncs nightly. Users then add the marketplace once (`/plugin marketplace add anthropics/claude-plugins-community`) and install `vocabloot-deck-kit@claude-community`. The official marketplace (`claude-plugins-official`) is curated by Anthropic with no application process. Until approval, the repo's own marketplace works: `claude plugin marketplace add vaazh-studios/vocabloot-deck-kit`, then `claude plugin install vocabloot-deck-kit@vocabloot-deck-kit`.
