# German Greetings: review report

Deck `de-greetings`, en-US → de, A1, 20 cards. Generated 2026-09-16 with prompt card-2.

## Needs a look (0)

Nothing flagged.


## Cards

### Hallo · hello

- /ˈhalo/ · interjection
- sticker: sticker (a person smiling and waving hello)
- Hallo, ich bin Anna.
  Hello, I am Anna.
  tokens: Hallo→hallo (hello) · ich→ich (I) · bin→sein (am) · Anna→Anna (Anna)
- Hallo! Schön, dich zu sehen.
  Hello! Nice to see you.
  tokens: Hallo→hallo (hello) · Schön→schön (nice, lovely) · dich→du (you) · zu→zu (to) · sehen→sehen (to see)

### Guten Morgen · good morning

- /ˌɡuːtn̩ ˈmɔʁɡn̩/ · phrase
- sticker: symbolic (a rising sun over a flat horizon in warm morning light)
- Guten Morgen! Hast du gut geschlafen?
  Good morning! Did you sleep well?
  tokens: Guten→gut (good) · Morgen→Morgen (morning) · Hast→haben (have, did) · du→du (you) · gut→gut (well) · geschlafen→schlafen (slept)
- Guten Morgen, Frau Weber.
  Good morning, Mrs Weber.
  tokens: Guten→gut (good) · Morgen→Morgen (morning) · Frau→Frau (Mrs, woman) · Weber→Weber (Weber)

### Guten Tag · hello (formal); good day

- /ˌɡuːtn̩ ˈtaːk/ · phrase
- sticker: symbolic (a bright sun high in a clear blue sky)
- Guten Tag, wie kann ich Ihnen helfen?
  Hello, how can I help you?
  tokens: Guten→gut (good) · Tag→Tag (day) · wie→wie (how) · kann→können (can) · ich→ich (I) · Ihnen→Sie (you (formal)) · helfen→helfen (to help)
- Guten Tag, Herr Schmidt!
  Good day, Mr Schmidt!
  tokens: Guten→gut (good) · Tag→Tag (day) · Herr→Herr (Mr, gentleman) · Schmidt→Schmidt (Schmidt)

### Guten Abend · good evening

- /ˌɡuːtn̩ ˈaːbn̩t/ · phrase
- sticker: symbolic (a setting sun low on the horizon under an orange evening sky)
- Guten Abend! Der Tisch ist fertig.
  Good evening! The table is ready.
  tokens: Guten→gut (good) · Abend→Abend (evening) · Der→der (the) · Tisch→Tisch (table) · ist→sein (is) · fertig→fertig (ready)
- Guten Abend, meine Damen und Herren.
  Good evening, ladies and gentlemen.
  tokens: Guten→gut (good) · Abend→Abend (evening) · meine→mein (my) · Damen→Dame (ladies) · und→und (and) · Herren→Herr (gentlemen)

### Gute Nacht · good night

- /ˌɡuːtə ˈnaxt/ · phrase
- sticker: symbolic (a crescent moon with three small stars)
- Gute Nacht und schlaf gut!
  Good night and sleep well!
  tokens: Gute→gut (good) · Nacht→Nacht (night) · und→und (and) · schlaf→schlafen (sleep) · gut→gut (well)
- Ich gehe ins Bett. Gute Nacht!
  I am going to bed. Good night!
  tokens: Ich→ich (I) · gehe→gehen (go, am going) · ins→in das (into the) · Bett→Bett (bed) · Gute→gut (good) · Nacht→Nacht (night)

### Tschüss · bye

- /tʃʏs/ · interjection
- sticker: sticker (a person walking away and waving goodbye)
- Tschüss, bis morgen!
  Bye, see you tomorrow!
  tokens: Tschüss→tschüss (bye) · bis→bis (until, see you) · morgen→morgen (tomorrow)
- Ich muss los. Tschüss!
  I have to go. Bye!
  tokens: Ich→ich (I) · muss→müssen (must, have to) · los→los (off, away) · Tschüss→tschüss (bye)

### Auf Wiedersehen · goodbye (formal)

- /aʊ̯f ˈviːdɐˌzeːən/ · phrase
- sticker: symbolic (a hand waving goodbye from a train window)
- Auf Wiedersehen und vielen Dank!
  Goodbye and thank you very much!
  tokens: Auf→auf (until, on) · Wiedersehen→Wiedersehen (seeing again, goodbye) · und→und (and) · vielen→viel (many, much) · Dank→Dank (thanks)
- Auf Wiedersehen, Frau Müller.
  Goodbye, Mrs Müller.
  tokens: Auf→auf (until, on) · Wiedersehen→Wiedersehen (seeing again, goodbye) · Frau→Frau (Mrs, woman) · Müller→Müller (Müller)

### Bis später · see you later

- /bɪs ˈʃpɛːtɐ/ · phrase
- sticker: symbolic (a round wall clock with a small waving hand beside it)
- Bis später, ich komme um sechs.
  See you later, I am coming at six.
  tokens: Bis→bis (until, see you) · später→spät (later) · ich→ich (I) · komme→kommen (come, am coming) · um→um (at) · sechs→sechs (six)
- Okay, bis später!
  Okay, see you later!
  tokens: Okay→okay (okay) · bis→bis (until, see you) · später→spät (later)

### Bis morgen · see you tomorrow

- /bɪs ˈmɔʁɡn̩/ · phrase
- sticker: symbolic (a desk calendar flipping to the next day)
- Bis morgen in der Schule!
  See you tomorrow at school!
  tokens: Bis→bis (until, see you) · morgen→morgen (tomorrow) · in→in (in, at) · der→der (the) · Schule→Schule (school)
- Gute Nacht, bis morgen!
  Good night, see you tomorrow!
  tokens: Gute→gut (good) · Nacht→Nacht (night) · bis→bis (until, see you) · morgen→morgen (tomorrow)

### Wie geht's? · how are you? (informal)

- /viː ˈɡeːts/ · phrase
- sticker: symbolic (a speech bubble with a question mark beside a smiling face)
- Hallo Tom, wie geht's?
  Hi Tom, how are you?
  tokens: Hallo→hallo (hello, hi) · Tom→Tom (Tom) · wie→wie (how) · geht's→gehen (goes it, is it going)
- Wie geht's dir heute?
  How are you today?
  tokens: Wie→wie (how) · geht's→gehen (goes it, is it going) · dir→du (to you) · heute→heute (today)

### Wie geht es Ihnen? · how are you? (formal)

- /viː ɡeːt ɛs ˈiːnən/ · phrase
- sticker: symbolic (a speech bubble with a question mark above a polite handshake)
- Guten Tag, Frau Berg. Wie geht es Ihnen?
  Hello, Mrs Berg. How are you?
  tokens: Guten→gut (good) · Tag→Tag (day) · Frau→Frau (Mrs, woman) · Berg→Berg (Berg) · Wie→wie (how) · geht→gehen (goes) · es→es (it) · Ihnen→Sie (you (formal))
- Wie geht es Ihnen heute, Herr Doktor?
  How are you today, doctor?
  tokens: Wie→wie (how) · geht→gehen (goes) · es→es (it) · Ihnen→Sie (you (formal)) · heute→heute (today) · Herr→Herr (Mr, gentleman) · Doktor→Doktor (doctor)

### Mir geht es gut · I am fine

- /miːɐ̯ ɡeːt ɛs ˈɡuːt/ · phrase
- sticker: symbolic (a thumbs-up hand)
- Danke, mir geht es gut.
  Thanks, I am fine.
  tokens: Danke→danke (thanks) · mir→ich (to me) · geht→gehen (goes) · es→es (it) · gut→gut (well, good)
- Mir geht es gut, und dir?
  I am fine, and you?
  tokens: Mir→ich (to me) · geht→gehen (goes) · es→es (it) · gut→gut (well, good) · und→und (and) · dir→du (to you)

### danke · thank you

- /ˈdaŋkə/ · interjection
- sticker: sticker (a person handing flowers to say thank you)
- Danke für die Blumen!
  Thank you for the flowers!
  tokens: Danke→danke (thank you) · für→für (for) · die→der (the) · Blumen→Blume (flowers)
- Danke, das ist sehr nett.
  Thanks, that is very kind.
  tokens: Danke→danke (thanks) · das→das (that) · ist→sein (is) · sehr→sehr (very) · nett→nett (kind, nice)

### bitte · please; you are welcome

- /ˈbɪtə/ · interjection
- sticker: sticker (a person holding out an open hand, asking politely)
- Ein Wasser, bitte.
  A water, please.
  tokens: Ein→ein (a) · Wasser→Wasser (water) · bitte→bitte (please)
- Danke! Bitte, gern geschehen.
  Thanks! You are welcome.
  tokens: Danke→danke (thanks) · Bitte→bitte (you are welcome) · gern→gern (gladly) · geschehen→geschehen (done, happened)

### die Entschuldigung · excuse me; sorry

- /ɛntˈʃʊldɪɡʊŋ/ · noun · feminine · plural Entschuldigungen
- sticker: symbolic (a hand raised, palm out, in apology)
- Entschuldigung, wo ist der Bahnhof?
  Excuse me, where is the train station?
  tokens: Entschuldigung→Entschuldigung (excuse me, sorry) · wo→wo (where) · ist→sein (is) · der→der (the) · Bahnhof→Bahnhof (train station)
- Entschuldigung, das war mein Fehler.
  Sorry, that was my mistake.
  tokens: Entschuldigung→Entschuldigung (sorry, excuse me) · das→das (that) · war→sein (was) · mein→mein (my) · Fehler→Fehler (mistake)

### willkommen · welcome

- /vɪlˈkɔmən/ · interjection
- sticker: sticker (a person opening a door to welcome someone in)
- Willkommen in Berlin!
  Welcome to Berlin!
  tokens: Willkommen→willkommen (welcome) · in→in (in, to) · Berlin→Berlin (Berlin)
- Herzlich willkommen bei uns!
  A warm welcome to our place!
  tokens: Herzlich→herzlich (warmly, heartily) · willkommen→willkommen (welcome) · bei→bei (at, with) · uns→wir (us)

### Ich heiße · my name is

- /ɪç ˈhaɪ̯sə/ · phrase
- sticker: sticker (a person pointing at themselves to say their name)
- Hallo, ich heiße Lena.
  Hello, my name is Lena.
  tokens: Hallo→hallo (hello) · ich→ich (I) · heiße→heißen (am called) · Lena→Lena (Lena)
- Ich heiße Paul. Und du?
  My name is Paul. And you?
  tokens: Ich→ich (I) · heiße→heißen (am called) · Paul→Paul (Paul) · Und→und (and) · du→du (you)

### Freut mich · nice to meet you

- /fʁɔʏ̯t mɪç/ · phrase
- sticker: symbolic (two hands in a friendly handshake)
- Freut mich, Sie kennenzulernen.
  Nice to meet you.
  tokens: Freut→freuen (pleases) · mich→ich (me) · Sie→Sie (you (formal)) · kennenzulernen→kennenlernen (to get to know)
- Ich bin Max. Freut mich!
  I am Max. Nice to meet you!
  tokens: Ich→ich (I) · bin→sein (am) · Max→Max (Max) · Freut→freuen (pleases) · mich→ich (me)

### Alles Gute · all the best

- /ˈaləs ˈɡuːtə/ · phrase
- sticker: symbolic (a wrapped gift with a bow and a little confetti)
- Alles Gute zum Geburtstag!
  Happy birthday!
  tokens: Alles→alles (everything, all) · Gute→gut (good) · zum→zu dem (for the, to the) · Geburtstag→Geburtstag (birthday)
- Alles Gute für die Prüfung!
  All the best for the exam!
  tokens: Alles→alles (everything, all) · Gute→gut (good) · für→für (for) · die→der (the) · Prüfung→Prüfung (exam)

### Schönen Tag noch · have a nice day

- /ˈʃøːnən taːk nɔx/ · phrase
- sticker: symbolic (a bright sun above a waving hand)
- Danke, schönen Tag noch!
  Thanks, have a nice day!
  tokens: Danke→danke (thanks) · schönen→schön (nice) · Tag→Tag (day) · noch→noch (still, yet)
- Schönen Tag noch, Herr Fischer!
  Have a nice day, Mr Fischer!
  tokens: Schönen→schön (nice) · Tag→Tag (day) · noch→noch (still, yet) · Herr→Herr (Mr, gentleman) · Fischer→Fischer (Fischer)

